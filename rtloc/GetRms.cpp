...
//luca
#if 1
float GetRms (/*int *imean*/Vect3D *mean, GridDesc *Pgrid, GridDesc *Sgrid, struct Pick *pick, int pickid, GridDesc *Grid, struct Control *params)
{
	int n, m=0;
	float tta, ttb;
	float timea, timeb;
...
tta = ReadAbsInterpGrid3d(&(Pgrid[pick[pickid].statid]), mean->x, mean->y, mean->z);
...
ttb = ReadAbsInterpGrid3d(&(Pgrid[pick[n].statid]), mean->x, mean->y, mean->z);
